...
import os
import streamlit as st
from rag_helper_utility_push import process_document_to_chroma_db, answer_question, get_horoscope_chart_data
...
if st.button("Generate Horoscope Chart"):
    chart_data = get_horoscope_chart_data(year, month, date, hours, minutes, seconds, latitude, longitude, timezone)
    st.write(chart_data)  # Print the chart data

user_question = st.text_area("Ask your question about the knowledgebase")
if st.button("Answer"):
    chart_data = get_horoscope_chart_data(year, month, date, hours, minutes, seconds, latitude, longitude, timezone)
    answer = answer_question(user_question, chart_data)
    st.markdown("JyotBot says")
    st.markdown(answer)
